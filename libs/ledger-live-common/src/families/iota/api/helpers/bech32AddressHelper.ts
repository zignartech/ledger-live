...
import { IBech32AddressDetails } from "../models";
...
export class Bech32AddressHelper {
  ...
  private static buildAddressFromTypes(
    ...
    if (address.type === ED25519_ADDRESS_TYPE)
      hex = HexHelper.stripPrefix(address.pubKeyHash);
    else if (address.type === ALIAS_ADDRESS_TYPE)
      hex = HexHelper.stripPrefix(address.aliasId);
    else if (address.type === NFT_ADDRESS_TYPE)
      hex = HexHelper.stripPrefix(address.nftId);
  ...
  private static typeLabel(addressType?: number): string | undefined {
    if (addressType === ED25519_ADDRESS_TYPE) return "Ed25519";
    else if (addressType === ALIAS_ADDRESS_TYPE) return "Alias";
    else if (addressType === NFT_ADDRESS_TYPE) return "NFT";
  }
}
