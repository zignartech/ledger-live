...
import { IBech32AddressDetails, IInput, IOutput } from "../models";
...
export class TransactionsHelper {
  public static async getInputsAndOutputs(
    ...
    if (block?.payload?.type === TRANSACTION_PAYLOAD_TYPE) {
      ...
      const transactionId =
        TransactionsHelper.computeTransactionIdFromTransactionPayload(payload);
      ...
      for (let i = 0; i < unlocks.length; i++) {
        ...
        if (unlock.type === SIGNATURE_UNLOCK_TYPE) signatureUnlock = unlock;
        else {
          let refUnlockIdx = i;
          // unlock references can be transitive,
          // so we need to follow the path until we find the signature
          do {
            const referenceUnlock = payload.unlocks[
              refUnlockIdx
            ] as IReferenceUnlock;
            signatureUnlock = payload.unlocks[
              referenceUnlock.reference
            ] as ISignatureUnlock;
        ...
        const hex = Converter.bytesToHex(
          new Ed25519Address(
            Converter.hexToBytes(signatureUnlock.signature.publicKey)
          ).toAddress()
        );
        unlockAddresses.push(Bech32AddressHelper.buildAddress(_bechHrp, hex));
      }
      ...
      for (let i = 0; i < payload.essence.inputs.length; i++) {
        ...
        const response = await apiClient.output(outputId);
        if (response.output && response.metadata) {
          outputDetails = {
            output: response.output,
            metadata: response.metadata,
          };
          amount = Number(response.output.amount);
        }
      ...
      for (let i = 0; i < payload.essence.outputs.length; i++) {
        ...
        if (payload.essence.outputs[i].type === TREASURY_OUTPUT_TYPE) {
          ...
          const output = payload.essence.outputs[i] as
            | IBasicOutput
            | IFoundryOutput
            | IAliasOutput
            | INftOutput;

          const address: IBech32AddressDetails =
            TransactionsHelper.bechAddressFromAddressUnlockCondition(
              output.unlockConditions,
              _bechHrp,
              output.type
            );

          const isRemainder = inputs.some(
            (input) => input.address.bech32 === address.bech32
          );
  ...
  public static sortInputsAndOuputsByIndex(items: IInput[] | IOutput[]) {
    ...
    items.sort((a: any, b: any) => {
      ...
      return (
        Number.parseInt(firstFormattedIndex, 16) -
        Number.parseInt(secondFormattedIndex, 16)
      );
    });
  }
   ...
   */
  public static buildIdHashForAliasOrNft(
    aliasOrNftId: string,
    outputId: string
  ): string {
  ...
  public static computeStorageRentBalance(
    outputs: OutputTypes[],
    rentStructure: IRent
  ): number {
    const outputsWithoutSdruc = outputs.filter((output) => {
      if (output.type === TREASURY_OUTPUT_TYPE) return false;
    ...
    const rentBalance = outputsWithoutSdruc.reduce(
      (acc, output) =>
        acc + TransactionHelper.getStorageDeposit(output, rentStructure),
      0
    );
    return rentBalance;
  }
  ...
  public static isParticipationEventOutput(output: OutputTypes): boolean {
    if (output.type === BASIC_OUTPUT_TYPE) {
      ...
      if (tagFeature) return tagFeature.tag === HEX_PARTICIPATE;
    }
    return false;
  }
  ...
  private static bechAddressFromAddressUnlockCondition(
    ...
    if (outputType === BASIC_OUTPUT_TYPE || outputType === NFT_OUTPUT_TYPE)
      unlockCondition = unlockConditions
        ?.filter((ot) => ot.type === ADDRESS_UNLOCK_CONDITION_TYPE)
      ...
      if (
        ...
        unlockCondition = unlockConditions
          ?.filter((ot) => ot.type === GOVERNOR_ADDRESS_UNLOCK_CONDITION_TYPE)
          .map((ot) => ot as IGovernorAddressUnlockCondition)[0];
    } else if (outputType === FOUNDRY_OUTPUT_TYPE)
      unlockCondition = unlockConditions
        ?.filter((ot) => ot.type === IMMUTABLE_ALIAS_UNLOCK_CONDITION_TYPE)
